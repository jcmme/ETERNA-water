// Water Button — adapted from OriginKit for ETERNA Water.
// Glass button with a real 1D shallow-water simulation on <canvas>: the
// surface tips and sloshes when the cursor sweeps it, rather than just
// bobbing in place, and settles/sleeps once still to avoid idle CPU use.
"use client";
 ...
 *
 * Water accelerates down the slope of its own surface, and mass moves between
 * columns by flux — water that leaves one column arrives in another, so the
 * level tips, piles against the far wall and sloshes back. Mass is conserved
...
/* Spray: droplets torn off the surface (plain ballistics) where the cursor
 * rips through or a wave outruns itself, poured back wherever they land. */
const MAX_DROPS = 96;
...
/**
 * Glass Button — a pane of glass part-filled with water, level and dead
 * still. Run the cursor over it and the surface is dragged and split into
 * ripples that travel out, bounce off the ends and cross back through each
 * other; stop, and it levels off and freezes again.
 */
export function WaterButton(props: WaterButtonProps) {
  const {
